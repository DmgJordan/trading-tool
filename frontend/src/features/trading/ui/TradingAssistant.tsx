...
import { useOHLCV } from '@/features/trading/model/queries';
import { usePreferencesStore } from '@/features/preferences/model/store';
import { getProfileLabel, getProfileDescription } from '@/shared/lib/ui';
...
export default function TradingAssistant({
  ...
  const ohlcvQuery = useOHLCV(selectedTicker, DEFAULT_TIMEFRAME);
  const latestClose = ohlcvQuery.data?.last_200.at(-1)?.[4];
        ...
        {selectedTicker && (
          <p className="text-sm text-gray-500 mt-3">
            {ohlcvQuery.isFetching
              ? `Chargement des données ${DEFAULT_TIMEFRAME}...`
              : typeof latestClose === 'number'
                ? `Clôture ${DEFAULT_TIMEFRAME} : ${latestClose.toLocaleString(
                    'fr-FR',
                    {
