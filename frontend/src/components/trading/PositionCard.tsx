...
export default function PositionCard({
  ...
  const sideColor = () => {
    ...
    if (normalizedSide === 'Long' || normalizedSide === 'Achat') return 'text-green-600 bg-green-50';
    if (normalizedSide === 'Short' || normalizedSide === 'Vente') return 'text-red-600 bg-red-50';
    ...
    <div className={`bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow ${className}`}>
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${sideColor()}`}>
            {normalizeSide(side)}
          </span>
        </div>
        <div className="text-right">
          <div className={`text-lg font-semibold ${
            isPositivePnl ? 'text-green-600' :
            isNegativePnl ? 'text-red-600' : 'text-gray-900'
          }`}>
            {formatCurrency(pnl)}
          </div>
          {pnlPercentage !== undefined && Number.isFinite(pnlPercentage) && (
            <div className={`text-sm ${
              pnlPercentage > 0 ? 'text-green-600' :
              pnlPercentage < 0 ? 'text-red-600' : 'text-gray-500'
            }`}>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        ...
        <div>
          ...
          <p className="font-medium text-gray-900">{formatNumber(entryPrice, 2)}</p>
        </div>
        <div>
          <p className="text-gray-500 mb-1">Prix actuel</p>
          <p className="font-medium text-gray-900">{formatNumber(markPrice, 2)}</p>
        </div>
        <div>
          <p className="text-gray-500 mb-1">Variation</p>
          <div className="flex items-center gap-1">
            {entryPrice && markPrice && Number.isFinite(Number(entryPrice)) && Number.isFinite(Number(markPrice)) ? (
              (() => {
                const change = ((Number(markPrice) - Number(entryPrice)) / Number(entryPrice)) * 100;
                return (
