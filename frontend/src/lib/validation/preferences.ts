...
export const tradingStyleSchema = z.enum(['CONSERVATIVE', 'BALANCED', 'AGGRESSIVE'], {
...
export const tradingPreferencesUpdateSchema = tradingPreferencesSchema.partial();

// Schéma de validation pour le formulaire UI avec transformations
export const tradingPreferencesFormSchema = z.object({
  // Profil de trading
  risk_tolerance: riskToleranceSchema,
  investment_horizon: investmentHorizonSchema,
  trading_style: tradingStyleSchema,
  ...
  technical_indicators: z
    .array(z.string())
    .pipe(technicalIndicatorsSchema)
  ...
  message: 'Les paramètres de risque ne correspondent pas au style de trading choisi',
  path: ['trading_style']
});

// Types inférés des schémas
export type TradingPreferencesFormData = z.infer<typeof tradingPreferencesFormSchema>;
export type TradingPreferencesData = z.infer<typeof tradingPreferencesSchema>;
export type TradingPreferencesUpdateData = z.infer<typeof tradingPreferencesUpdateSchema>;
...
export const assetSymbolSchema = z
  ...
  .regex(/^[A-Z0-9]+$/, 'Seules les lettres majuscules et chiffres sont autorisés')
...
export const validateAssetSymbol = (symbol: string): { isValid: boolean; error?: string } => {
  try {
...
export const validateTechnicalIndicator = (indicator: string): { isValid: boolean; error?: string } => {
  try {
