...
export default function ExecuteTradeModal({
  ...
  const calculateRisk = () => {
    ...
    const riskPerToken = Math.abs(recommendation.entry_price - recommendation.stop_loss);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto ${className}`}>
        {/* Header */}
        <div className={`${colors.bg} ${colors.border} border-b-2 px-6 py-4`}>
          ...
          <p className={`text-sm ${colors.accent} mt-1`}>
            {recommendation.direction.toUpperCase()} {ticker} • Confiance {recommendation.confidence_level}%
          </p>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="font-semibold text-gray-900 mb-3">Informations du portefeuille</h3>
            {isLoadingPortfolio ? (
          ...
          <div className={`${colors.bg} ${colors.border} border-2 rounded-lg p-4`}>
            <h3 className={`font-semibold ${colors.text} mb-3`}>Résumé du trade</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <div className="text-gray-600">Valeur de la position</div>
                <div className="font-semibold">${positionValue.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-gray-600">Taille de la position</div>
                <div className="font-semibold">{positionSize.toFixed(6)} {ticker}</div>
              </div>
              <div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 border-opacity-50">
              <div className="grid grid-cols-4 gap-2 text-xs">
                <div className="text-center">
                  ...
                  <div className="font-semibold">${recommendation.entry_price.toLocaleString()}</div>
                </div>
                <div className="text-center">
                  <div className="text-red-600">Stop Loss</div>
                  <div className="font-semibold text-red-700">${recommendation.stop_loss.toLocaleString()}</div>
                </div>
                <div className="text-center">
                  <div className="text-green-600">TP1</div>
                  <div className="font-semibold text-green-700">${recommendation.take_profit_1.toLocaleString()}</div>
                </div>
                <div className="text-center">
                  <div className="text-green-600">TP3</div>
                  <div className="font-semibold text-green-700">${recommendation.take_profit_3.toLocaleString()}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-start space-x-2">
              ...
              <div className="text-sm text-yellow-800">
                <p className="font-semibold mb-2">Avertissements importants :</p>
                <ul className="space-y-1 text-xs">
                  <li>• Le trading comporte des risques de perte en capital</li>
                  <li>• Cette recommandation n'est pas un conseil financier</li>
                  <li>• Vous êtes responsable de vos décisions de trading</li>
                  <li>• Les ordres seront placés automatiquement sur Hyperliquid</li>
                  {useTestnet && <li>• Mode testnet activé - Aucun fonds réel ne sera utilisé</li>}
                </ul>
              </div>
            </div>
          </div>
