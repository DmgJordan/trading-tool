...
export default function RecommendationCard({
  ...
  const actionConfig = RECOMMENDATION_ACTIONS.find(
    a => a.value === recommendation.action
  );
  const riskConfig = RISK_LEVELS.find(
    r => r.value === recommendation.risk_level
  );

  const isPending = recommendation.status === 'PENDING';
  const isExpired = isRecommendationExpired(recommendation.expires_at);
  const isExpiringSoon = isRecommendationExpiringSoon(
    ...
    >
      <div className="p-6">
          ...
          <div className="flex items-center space-x-3">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${
                actionConfig?.value === 'BUY'
                  ? 'bg-green-500'
                  : actionConfig?.value === 'SELL'
                    ? 'bg-red-500'
                    : 'bg-blue-500'
              }`}
            >
            ...
            <div>
              ...
              <div className="flex items-center space-x-2">
                <span
                  className={`text-sm font-medium ${actionConfig?.color || 'text-gray-600'}`}
                >
                  {actionConfig?.icon}{' '}
                  {actionConfig?.label || recommendation.action}
                </span>
                {riskConfig && (
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${riskConfig.bgColor} ${riskConfig.color} font-medium`}
                  >
          ...
          <div>
            ...
            <p
              className={`text-lg font-bold ${
                recommendation.estimated_pnl >= 0
                  ? 'text-green-600'
                  : 'text-red-600'
              }`}
            >
              {formatPnL(
                recommendation.estimated_pnl,
                recommendation.estimated_pnl_percentage
              )}
            </p>
          </div>
        </div>
        ...
        {isExpanded &&
          ...
          Object.keys(recommendation.market_data).length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-200">
              <h4 className="text-sm font-medium text-gray-700 mb-2">
                Données de marché
              </h4>
              <div className="grid grid-cols-2 gap-3 text-xs">
                {Object.entries(recommendation.market_data)
                  .slice(0, 4)
                  .map(([key, value]) => (
