...
const normalizePortfolioSummary = (raw?: Record<string, unknown>): HyperliquidPortfolioSummary => {
  return {
    ...
    totalUnrealizedPnl: normalizeNumber(raw?.total_unrealized_pnl ?? raw?.totalUnrealizedPnl ?? raw?.totalUnrealizedPnL),
    withdrawableBalance: normalizeNumber(raw?.withdrawable_balance ?? raw?.withdrawableBalance ?? raw?.withdrawable),
    perpPositionCount: typeof raw?.perp_position_count === 'number'
      ? (raw?.perp_position_count as number)
      : typeof raw?.perpPositionCount === 'number'
        ? (raw?.perpPositionCount as number)
        : 0,
    assetPositionCount: typeof raw?.asset_position_count === 'number'
...
const normalizeFills = (raw?: Array<Record<string, unknown>>): HyperliquidFill[] => {
  if (!Array.isArray(raw)) return [];

  return raw.map(fill => ({
    time: typeof fill?.time === 'number' ? fill.time : undefined,
    symbol: typeof fill?.symbol === 'string' ? fill.symbol : (typeof fill?.coin === 'string' ? (fill.coin as string) : undefined),
    coin: typeof fill?.coin === 'string' ? fill.coin : undefined,
    side: typeof fill?.side === 'string' ? fill.side : (typeof fill?.dir === 'string' ? (fill.dir as string) : undefined),
    dir: typeof fill?.dir === 'string' ? fill.dir : undefined,
    sz: typeof fill?.sz === 'string' ? fill.sz : (typeof fill?.size === 'string' ? (fill.size as string) : undefined),
    size: typeof fill?.size === 'string' ? fill.size : undefined,
    px: typeof fill?.px === 'string' ? fill.px : (
...
const normalizeOpenOrders = (raw?: Array<Record<string, unknown>>): HyperliquidOpenOrder[] => {
  if (!Array.isArray(raw)) return [];

  return raw.map(order => ({
    coin: typeof order?.coin === 'string' ? order.coin : undefined,
    symbol: typeof order?.symbol === 'string' ? order.symbol : undefined,
    sz: typeof order?.sz === 'string' ? order.sz : (typeof order?.size === 'string' ? (order.size as string) : undefined),
    size: typeof order?.size === 'string' ? order.size : undefined,
    px: typeof order?.px === 'string' ? order.px : (
    ...
    side: typeof order?.side === 'string' ? order.side : (typeof order?.dir === 'string' ? (order.dir as string) : undefined),
    dir: typeof order?.dir === 'string' ? order.dir : undefined,
    isTrigger: typeof order?.isTrigger === 'boolean' ? order.isTrigger : undefined,
    tif: typeof order?.tif === 'string' ? order.tif : undefined,
    triggerPx: typeof order?.triggerPx === 'string' ? order.triggerPx : undefined,
    reduceOnly: typeof order?.reduceOnly === 'boolean' ? order.reduceOnly : undefined,
    cloid: typeof order?.cloid === 'string' ? order.cloid : undefined,
    oid: typeof order?.oid === 'number' ? order.oid : undefined,
  }));
};

const normalizeUserState = (raw?: Record<string, unknown> | null): HyperliquidUserState | null => {
  if (!raw) return null;

  const marginSummaryRaw = raw.marginSummary as Record<string, unknown> | undefined;
  const crossMarginSummaryRaw = raw.crossMarginSummary as Record<string, unknown> | undefined;
  const normalizeSummary = (summary?: Record<string, unknown>): Record<string, unknown> | null => (
  ...
  return {
    ...
    assetPositions: Array.isArray(raw.assetPositions)
      ? (raw.assetPositions as HyperliquidPerpPosition[])
      : perpPositionsRaw,
    withdrawables: typeof raw.withdrawables === 'object' && raw.withdrawables !== null
...
const normalizeSpotUserState = (raw?: Record<string, unknown> | null): HyperliquidSpotUserState | null => {
  if (!raw) return null;
  ...
  return {
    ...
    assetPositions: spotAssetPositions as HyperliquidSpotUserState['assetPositions'],
  };
};

const normalizeUserInfo = (raw: HyperliquidUserInfoRaw | undefined, timestamp: string): HyperliquidUserInfoData | undefined => {
  if (!raw?.wallet_address || !raw.network) {
    return undefined;
  }

  const retrievedAt = typeof raw.retrieved_at === 'string' ? raw.retrieved_at : timestamp;
...
export const hyperliquidApi = {
  async getUserInfo(options: HyperliquidUserInfoOptions = {}): Promise<HyperliquidConnectorResponse> {
    const response = await apiClient.post<HyperliquidConnectorResponseRaw>('/connectors/user-info', {
