...
export default function TradeCard({
  ...
  const riskAmount = Math.abs(recommendation.entry_price - recommendation.stop_loss);
  const reward1 = Math.abs(recommendation.take_profit_1 - recommendation.entry_price);
  const reward2 = Math.abs(recommendation.take_profit_2 - recommendation.entry_price);
  const reward3 = Math.abs(recommendation.take_profit_3 - recommendation.entry_price);
  ...
  return (
    <div className={`${colors.bg} ${colors.border} border-2 rounded-xl p-6 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className={`text-sm ${colors.accent}`}>
              {recommendation.timeframe} • {recommendation.portfolio_percentage}% portefeuille
            </p>
          </div>
        </div>
        ...
        <div className="text-center">
          <div className={`text-xs font-medium ${colors.text} mb-1`}>Confiance</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        {/* Prix d'entrée */}
        <div className="text-center">
          <div className={`text-xs font-medium ${colors.accent} mb-1`}>Entrée</div>
        ...
        <div className="text-center">
          <div className="text-xs font-medium text-green-600 mb-1">TP Principal</div>
        ...
        {/* Ratio R/R */}
        <div className="text-center">
          <div className={`text-xs font-medium ${colors.accent} mb-1`}>Ratio R/R</div>
          <div className={`text-lg font-bold ${colors.text}`}>
      ...
      <div className="bg-white bg-opacity-50 rounded-lg p-3 mb-4">
        <div className="text-xs font-medium text-gray-600 mb-2">Take Profits Échelonnés</div>
      ...
      <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
        <div className="flex items-start space-x-2">
          ...
          <p className="text-xs text-yellow-800">
            <strong>Attention :</strong> Le trading comporte des risques de perte.
            Cette recommandation n'est pas un conseil financier.
            Risque : {((riskAmount / recommendation.entry_price) * 100).toFixed(1)}% par position.
          </p>
        </div>
      </div>
    </div>
